...
class CLICommandsMixin:
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Gaia Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Gaia Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Gaia Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Gaia Agent",
