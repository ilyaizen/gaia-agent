"""Shell completion script generation for Gaia CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Gaia Agent bash completion
# Add to ~/.bashrc:
#   eval "$(gaia completion bash)"
...
complete -F _gaia_completion gaia
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef gaia
# Gaia Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(gaia completion zsh)"
...
_gaia() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_gaia_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'gaia command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Gaia Agent fish completion",
        "# Add to your config:",
        "#   gaia completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __gaia_profiles",
        "    echo default",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c gaia -f -s p -l profile"
